...
- (void)commonInit {
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [_collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(DYCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
...
- (void)receiveMemoryWarningHander:(NSNotificationCenter *)noti {
    __weak typeof(self) weakSelf = self;
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<DYScrollPageViewChildViewControllerDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf) {
            if (childVc != strongSelf.currentChildViewController) {
                [_childVcsDic removeObjectForKey:key];
                [DYContentView removeChildVc:childVc];
            }
        }
    }];
}
...
//给外界刷新视图的方法
- (void)reload {
    [self.childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<DYScrollPageViewChildViewControllerDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        [DYContentView removeChildVc:childVc];
        childVc = nil;
    }];
...
- (void)willAppearWithIndex:(NSInteger)index {
    UIViewController<DYScrollPageViewChildViewControllerDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(dy_viewWillAppearForIndex:)]) {
            [controller dy_viewWillAppearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)didAppearWithIndex:(NSInteger)index {
    UIViewController<DYScrollPageViewChildViewControllerDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(dy_viewDidAppearForIndex:)]) {
            [controller dy_viewDidAppearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)willDisappearWithIndex:(NSInteger)index {
    UIViewController<DYScrollPageViewChildViewControllerDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(dy_viewWillDisappearForIndex:)]) {
            [controller dy_viewWillDisappearForIndex:index];
        }
        if (_needManageLifeCycle) {
...
- (void)didDisappearWithIndex:(NSInteger)index {
    UIViewController<DYScrollPageViewChildViewControllerDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if ([controller respondsToSelector:@selector(dy_viewDidDisappearForIndex:)]) {
            [controller dy_viewDidDisappearForIndex:index];
        }
        if (_needManageLifeCycle) {
            [controller endAppearanceTransition];
        }
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    _currentChildViewController = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", (long)indexPath.row]];
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (_currentChildViewController == nil) {
            ...
            if (!_currentChildViewController || ![_currentChildViewController conformsToProtocol:@protocol(DYScrollPageViewChildViewControllerDelegate)]) {
                NSAssert(NO, @"子控制器必须遵守DYScrollPageViewChildViewControllerDelegate协议");
            }
            // 设置当前下标
            _currentChildViewController.dy_currentIndex = indexPath.row;
            [self.childVcsDic setValue:_currentChildViewController forKey:[NSString stringWithFormat:@"%ld", (long)indexPath.row]];
        } else {
    ...
    if (_currentChildViewController.dy_scrollViewController != self.parentViewController) {
        [self.parentViewController addChildViewController:_currentChildViewController];
    }
    _currentChildViewController.view.frame = cell.contentView.bounds;
    ...
    if (_isLoadFirstView) { // 第一次加载cell? 不会调用endDisplayCell
        ...
        if (isFirstLoaded) {
            // viewDidLoad
            if ([_currentChildViewController respondsToSelector:@selector(dy_viewDidLoadForIndex:)]) {
                [_currentChildViewController dy_viewDidLoadForIndex:indexPath.row];
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.forbidTouchToAdjustPosition) {
        if (_currentIndex == indexPath.row) {// 没有滚动完成
            ...
            if (_needManageLifeCycle) {
                UIViewController<DYScrollPageViewChildViewControllerDelegate> *currentVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", _oldIndex]];
                // 开始出现
                [currentVc beginAppearanceTransition:YES animated:NO];
                
                UIViewController<DYScrollPageViewChildViewControllerDelegate> *oldVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", indexPath.row]];
                // 开始消失
...
- (DYCollectionView *)collectionView {
    if (_collectionView == nil) {
        DYCollectionView *collectionView = [[DYCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.collectionViewLayout];
...
- (NSMutableDictionary<NSString *,UIViewController<DYScrollPageViewChildViewControllerDelegate> *> *)childVcsDic {
    if (!_childVcsDic) {
